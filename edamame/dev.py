...
def station_info(pref=14, year=2020, target="OX", PATH=""):
    ...
    # Warningが出るが、問題ないらしい。
    df2["latitude"] = df["緯度_度"] + df["緯度_分"] / 60 + df["緯度_秒"] / 3600
    df2["longitude"] = df["経度_度"] + df["経度_分"] / 60 + df["経度_秒"] / 3600
...
def draw(target, h, ax, gX, gY, dfs, stations, vcolorify, shape=None):
    if target == "WS":
        # 1. グリッド点の処理
        ...
        e = ax.scatter(
            points[:, 0],
            points[:, 1],
            color=vcolorify((sZx**2 + sZy**2) ** 0.5),
        )
        elems.append(e)
        for df in dfs:
